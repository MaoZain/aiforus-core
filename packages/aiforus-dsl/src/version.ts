...
/**
 * A document that may carry a DSL contract-version stamp. `@aiforus/dsl` does
...
function versionOf(
  ...
  if (typeof raw !== 'string' || !isWellFormedDslVersion(raw)) {
    throw new Error(`@aiforus/dsl: invalid ${key} stamp ${JSON.stringify(raw)} (expected "x.y.z")`);
  }
  return raw;
}
...
function crossLineError(key: string, otherKey: string): Error {
  return new Error(
    `@aiforus/dsl: object carries "${otherKey}" but no "${key}" — a misrouted ` +
...
function noRuntimeEpochError(): Error {
  return new Error(
    `@aiforus/dsl: object carries no version stamp; the runtime line has no ` +
...
function runLadder(
  ...
  for (let step = 0; step < ladder.length + 1; step++) {
    ...
    if (!next) {
      throw new Error(`@aiforus/dsl: no migration path from "${version}" to "${targetVersion}"`);
    }
  ...
  if (version !== targetVersion) {
    throw new Error(
      `@aiforus/dsl: migration ladder did not reach "${targetVersion}" (stuck at "${version}")`,
    );
  }
  return current;
}
