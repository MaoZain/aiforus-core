...
function fail(el: Raw, field: string, expected: string, value: unknown = el[field]): never {
  throw new Error(
    `@aiforus/dsl: cannot normalize ${String(el.type)} element ${JSON.stringify(el.id)}: ` +
...
export function normalizeElement(el: unknown): PPTElement {
  if (!isObject(el))
    throw new Error(
      `@aiforus/dsl: cannot normalize element: expected an object, got ${JSON.stringify(el)}`,
    );
  switch (el.type) {
    case 'text':
    ...
    default:
      throw new Error(
        `@aiforus/dsl: cannot normalize element ${JSON.stringify(el.id)}: ` +
